-- Fix: Prevent trigger from creating duplicate organizations for staff members
-- When a staff member is created via the admin panel, the backend creates the user first
-- This trigger should skip organization creation if the user already exists
...
BEGIN
  -- Check if user already exists (created by backend for staff)
  SELECT id INTO existing_user_id
  FROM public.users
  WHERE auth_id = new.id;

  -- If user already exists, skip organization creation
  IF existing_user_id IS NOT NULL THEN
    RAISE NOTICE 'User already exists with auth_id %, skipping organization creation', new.id;
    RETURN new;
  END IF;
