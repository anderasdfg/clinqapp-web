-- Migration: replace dni with document_type + document_number on users table
-- Executed via Prisma.$executeRawUnsafe (pooler-compatible)
...
-- 2. Migrate existing data: copy dni value into document_number, default type to DNI
UPDATE users SET document_type = 'DNI', document_number = dni WHERE dni IS NOT NULL;

-- 3. Create index on document_number for fast lookup
CREATE INDEX IF NOT EXISTS idx_users_document_number ON users(document_number);

-- 4. Drop old dni column and its index
DROP INDEX IF EXISTS idx_users_dni;
ALTER TABLE users DROP COLUMN IF EXISTS dni;

-- 5. Update the handle_new_user trigger to populate document_type and document_number
...
BEGIN
  ...
  INSERT INTO public.organizations (
    id, name, slug, email,
    is_temporary, onboarding_completed,
    notification_email, notification_whatsapp,
    send_reminders, reminder_hours_before,
    default_appointment_duration, appointment_interval,
    allow_online_booking, specialty,
    subscription_plan, subscription_status, updated_at
  ) VALUES (
    new_org_uuid, org_name, org_slug, new.email,
    true, false,
    true, false,
    true, 24,
    30, 0,
    false, 'PODIATRY',
    'FREE_TRIAL', 'TRIALING', NOW()
  ) RETURNING id INTO org_id;

  -- 2. Create User linked to Organization (with document fields)
  INSERT INTO public.users (
    id, auth_id, email, first_name, last_name,
    document_type, document_number,
    organization_id, role, email_verified, updated_at
  ) VALUES (
    gen_random_uuid(), new.id, new.email, user_first_name, user_last_name,
    user_document_type, user_document_number,
    org_id, 'OWNER', false, NOW()
  );
