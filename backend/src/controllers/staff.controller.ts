...
const createStaffSchema = z.object({
    ...
    role: z.enum(['PROFESSIONAL', 'OWNER', 'RECEPTIONIST']).default('PROFESSIONAL'),
});
...
export const getStaff = async (req: AuthRequest, res: Response) => {
    try {
        ...
        // Role filter
        if (role && (role === 'PROFESSIONAL' || role === 'OWNER' || role === 'RECEPTIONIST')) {
            where.role = role;
        }
...
export const createStaffWithAuth = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (existingAuthUser) {
            return res.status(409).json({ error: 'El usuario de autenticación ya existe' });
        }
