...
interface AppointmentsState {
    ...
    checkAvailability: (professionalId: string, startTime: string, endTime: string, excludeId?: string) => Promise<boolean>;
}
...
export const useAppointmentsStore = create<AppointmentsState>((set, get) => ({
    ...
    updateAppointmentStatus: async (id, data) => {
        set({ isUpdating: true });
        try {
            const updatedAppointment = await appointmentsService.updateAppointmentStatus(id, data);
