...
export const basicClinicDataSchema = z.object({
  ...
  specialty: z
    .enum(["PODIATRY", "DENTISTRY", "AESTHETICS", "GENERAL"])
    .optional()
    .default("PODIATRY"),
...
export const businessHourSchema = z
  ...
  .refine(
    (data: { enabled: boolean; startTime: string; endTime: string }) => {
...
export const businessHoursSchema = z.object({
  schedules: z
    ...
    .min(1, "Debe configurar al menos un día"),
});
...
export const paymentMethodSchema = z
  ...
  .refine(
    (data: { type: string; otherName?: string | null }) => {
      if (data.type === "OTHER") {
    ...
    {
      message: "Debe especificar el nombre del método de pago personalizado",
      path: ["otherName"],
    }
  );
...
export const consultationTypesSchema = z.object({
  types: z
    .array(z.enum(["IN_PERSON", "TELEMEDICINE", "HOME_VISIT"]))
...
export const serviceSchema = z.object({
  ...
  category: z
    .enum(["DIAGNOSTIC", "TREATMENT", "FOLLOWUP", "OTHER"])
    .default("OTHER"),
  duration: z
    .number()
